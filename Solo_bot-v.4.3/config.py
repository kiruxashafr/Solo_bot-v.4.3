...
# Платежные системы
# YooKassa
YOOKASSA_ENABLE = True  # Включить платежи через YooKassa (True — да, False — нет)
YOOKASSA_SHOP_ID = "1120574"  # ID магазина в YooKassa
YOOKASSA_SECRET_KEY = "live_-Jadn10O0l6D-bwwUyurtYlCy_Fe71sKFQx1eC65o5E"  # Секретный ключ YooKassa
EMAIL = "protonsvpn.ru"  # Домен почты для чеков (например, solonet.ru, полную почту не указывать)
SERVICE_NAME = "Пополнение баланса"  # Название услуги в чеке

# YooMoney
YOOMONEY_ENABLE = False  # Включить платежи через YooMoney (True — да, False — нет)
...
RENEWAL_PRICES = {
    "1": 149,  # 1 месяц — 100 рублей
    "3": 419,  # 3 месяца — 285 рублей
    "6": 799,  # 6 месяцев — 540 рублей
    "12": 1499,  # 12 месяцев — 1000 рублей
}

# Ограничения пользователей
TRIAL_CONFIG = {
    "duration_days": 7,              # срок действия
    "traffic_limit_gb": 100,         # трафик в ГБ
    "hwid_limit": 2,                 # лимит устройств, если актуально
}
...
# Сообщения в боте
NEWS_MESSAGE = "Приглашай друзей и получай 75 рублей за каждого на баланс! 🎁"  # Сообщение в профиле для пользователей с активной подпиской
...
NOTIFY_DELETE_DELAY = 600  # Отсрочка удаления подписки в минутах (0 — без отсрочки)
NOTIFY_RENEW_EXPIRED = True  # Продлевать подписку после истечения срока (True — да, False — нет)

NOTIFY_EXTRA_DAYS = 10  # Дополнительные дни для пробного периода
...
DISCOUNT_ACTIVE_HOURS = 72  # Срок действия скидки в часах
